alter table public.stg_events
  add column if not exists parsed_facts jsonb;
...
create index if not exists alerts_parsed_facts_gin on public.alerts using gin (parsed_facts);
create index if not exists logs_parsed_facts_gin on public.logs using gin (parsed_facts);
